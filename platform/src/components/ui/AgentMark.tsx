...
interface AgentMarkProps {
  id: AgentId;
  className?: string;
  /** Use the full-color 2.5D cube (onboarding / large placements). */
  variant?: '2d' | '25d';
}
...
/** Official Claude / Cursor / Codex brand marks. */
export function AgentMark({ id, className = '', variant = '2d' }: AgentMarkProps) {
  ...
  if (variant === '25d') {
    return (
      <svg viewBox="0 0 466.73 533.32" fill="none" className={className} aria-hidden="true">
        <path
          fill="#72716d"
  ...
  return (
    <span className={`inline-flex shrink-0 ${className}`} aria-hidden="true">
      <svg
        ...
        fill="none"
        className="agent-mark-cursor agent-mark-cursor-light h-full w-full">
        <path d={CURSOR_2D_PATH} fill="#26251e" />
      </svg>
      <svg
        viewBox="0 0 466.73 532.09"
        fill="none"
        className="agent-mark-cursor agent-mark-cursor-dark h-full w-full">
        <path d={CURSOR_2D_PATH} fill="#edecec" />
      </svg>
    </span>
  );
}
